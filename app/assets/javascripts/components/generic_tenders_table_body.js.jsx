var GenericTendersTableBody = React.createClass({
  render: function() {
    ...
    for (var i=0; i < this.props.tenders.length; i++) {
      ...
      rows.push(<GenericTenderRow refNo={this.props.tenders[i].ref_no} description={this.props.tenders[i].description} publishedDate={publishedDate} closingDate={closingDate} closingTime={closingTime} buyerCompanyName={this.props.tenders[i].buyer_company_name} watchPath={this.props.tenders[i].watch_path} unwatchPath={this.props.tenders[i].unwatch_path} watched={this.props.tenders[i].watched}/>);
    }
    return (
      <tbody>
        {rows}
