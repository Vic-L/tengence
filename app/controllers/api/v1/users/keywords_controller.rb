        ...
        begin
          ...
          if current_user.valid?
            if current_user.save
              NotifyViaSlack.delay.call(content: "#{current_user.email} updated keywords:\r\n#{current_user.keywords}")
              render :json => { :success => true }
            else
              NotifyViaSlack.delay.call(content: "<@vic-l> ERROR updating keywords by #{current_user.email}\r\n#{current_user.errors.full_messages.to_sentence}")
              render json: current_user.errors.full_messages.to_sentence, status: 500
            end
          else
            NotifyViaSlack.delay.call(content: "#{current_user.email} trying to be funny\r\n#{params[:keywords]}")
            render json: current_user.errors.full_messages.to_sentence, status: 400
          end
        rescue => e
          NotifyViaSlack.delay.call(content: "<@vic-l> ERROR api/v1/users/keywords_controller.rb\r\n#{e.message}\r\n#{e.backtrace.to_s}")
