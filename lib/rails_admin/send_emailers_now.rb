              ...
              User.read_only.each do |user|
                begin
                  ...
                  if Time.now.in_time_zone('Asia/Singapore').to_date.monday?
                    current_tenders_ref_nos = CurrentTender.where(ref_no: results_ref_nos).where("published_date >= ?", Time.now.in_time_zone('Asia/Singapore').to_date - 4.days).pluck(:ref_no) # get all tenders published from friday
                  else
